...
def basic_exponent(x , exponent, style='basicFwd'):
    ...
    if style == 'basicfwd':
        return np.where(x>0, xforward(x), 0) 
    elif style == 'basicrev':
        return np.where(x>0, xreverse(x), 0)
    elif style == 'basicmirrorfwd':
        return np.where(x>=0, xforward(x), -xforward(-x))
    elif style == 'basicmirrorrev':
        return np.where(x>=0, xreverse(x), -xreverse(-x))
    elif style == 'basicpassthrufwd':
        return np.where(x>0, xforward(x), x)
    elif style == 'basicpassthrurev':
        return np.where(x>0, xreverse(x), x) 
    else:
